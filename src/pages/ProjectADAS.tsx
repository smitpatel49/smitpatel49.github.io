...
export default function Page() {
  useHeadMeta({ title: 'Lane & Road-Sign Detection for Self-Driving', description: 'Perception, fusion, and control architecture, validated with a real OpenCV pipeline on synthetic scenes.', path: '/projects/adas' })
  return (
        ...
        <div className='grid grid-cols-1 gap-4 mt-8 max-w-3xl mx-auto'>
          <Card>
            ...
            <CardContent className='text-sm leading-relaxed space-y-2'>
              <p className='opacity-90'>Training a real segmentation/detection model needs a driving-scale image dataset and GPU time this project doesn't have. So instead of quoting borrowed benchmark numbers, this validates the part that's actually mine to prove: the <strong>fusion and control logic</strong> that turns perception output into a steering/speed decision, using a real, working <strong>classical computer-vision pipeline</strong> (Canny edges plus Hough transform for lanes, HSV color and contour shape for signs) on procedurally generated synthetic road scenes.</p>
              <p className='opacity-70 text-xs'>I generated the 60 test frames myself rather than pulling from a real driving dataset, partly since I didn't have the GPU time to train on one anyway, and partly since a lot of the public driving datasets come with usage restrictions I didn't want to get into for a portfolio demo. Every metric below still comes from actually running the pipeline against those frames.</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className='text-sm space-y-2'>
              <ArchitectureDiagram />
              <p className='opacity-90 text-xs'>The blue boxes are what this project actually implements and measures. The perception branches are swappable: classical CV here, a trained segmentation/detection model in production, with the same fusion and control code either side.</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className='text-sm space-y-3'>
              ...
              <p className='opacity-90'>Recall is high (color thresholding rarely <em>misses</em> a bright red/yellow shape), but precision lags: 28 false positives out of 112 detections, mostly blur and anti-aliasing artifacts that happen to fall in the right hue range. That gap is exactly why production perception uses a learned detector instead of hand-tuned color rules; a classifier learns shape and context, a hue mask doesn't.</p>
            </CardContent>
          </Card>
