...
export default function Page() {
  useHeadMeta({ title: 'Bank Marketing Classification', description: 'Calibrated XGBoost scoring model vs. a Random Forest baseline, tuned to a call-center capacity constraint.', path: '/projects/bank' })
  return (
        ...
        <div className='grid grid-cols-1 gap-4 mt-8 max-w-3xl mx-auto'>
          <Card>
            ...
            <CardContent className='text-sm leading-relaxed space-y-2'>
              <p className='opacity-90'>A call center can only reach a fraction of the customer list in a campaign window. <em>Which customers are actually worth calling?</em> That's a ranking and capacity problem, not just an accuracy problem, so the model is judged on lift at a realistic contact rate rather than on raw accuracy against a ~14% base rate.</p>
              <p className='opacity-70 text-xs'>I generated this dataset myself instead of pulling the well-known UCI bank marketing dataset it's modeled after. Some of that is licensing (I didn't want to republish someone else's data and call the results mine), and some of it is control: generating my own let me tune how separable the classes are, so the calibration story below stays real instead of trivial. The schema, the training run, and every metric are genuine.</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className='text-sm space-y-2'>
              <ul className='list-disc pl-5 space-y-2 opacity-90'>
                ...
                <li>Set the operating threshold from <strong>call-center capacity</strong> (top 15% of scored customers) instead of the default 0.5 cutoff, so the business constraint, not an arbitrary number, decides who gets called.</li>
              </ul>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className='text-sm space-y-3'>
              ...
              <p className='opacity-90'>Whether a customer said yes to a <em>previous</em> campaign dominates the model. That's unsurprising, but it means the single highest-leverage data investment isn't a new feature, it's better historical campaign-outcome logging.</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className='text-sm'>
              <pre className='overflow-x-auto text-xs md:text-sm'><code>{`bank_marketing/\n├─ data/ (synthetic sample .csv)\n├─ features.py\n├─ train.py\n├─ calibrate.py\n├─ serve/\n│  ├─ app.py (FastAPI)\n│  └─ schemas.py\n└─ Dockerfile`}</code></pre>
            </CardContent>
          </Card>
        </div>

        <ProjectPager slug='bank' />
      </div>
      </main>
    </div>
  )
